...
namespace stateObservation
{
  ...
  ObserverBase::StateVector TiltEstimator::oneStepEstimation_()
  {
    ...
    Vecotr3 yg = getMeasurement(k+1).tail(3);
    
    Vector3 y1 = R_S_C * yg - omega_S_C;
    Vector3 x1 = p_S_C.cross(y1) - dp_S_C;

    Vector6 dx_hat;
    dx_hat.head(3) = x1_hat.cross(y1) + gravityConstant * x2_hat - R_S_C * ya + alpha_ * (x1 - x1_hat);
    dx_hat.tail(3) = x2_hat.cross(y1 - beta_ * x2_hat.cross(x1 - x1_hat));
    
    ObserverBase::StateVector x_hat = getEstimatedState(k) + dx_hat * dt_;
